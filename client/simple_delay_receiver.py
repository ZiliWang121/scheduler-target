#!/usr/bin/python3
#python3 simple_delay_receiver.py 0.0.0.0 8888 1  #mptcp
#python3 simple_delay_receiver.py 0.0.0.0 8888 0  #tcp
import socket
import sys
import os
import threading
...
class ConnectionHandler(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            print(f"[Receiver] Starting to handle connection from 
{self.addr}")
            ...
            while True:
                ...
                # 【修改】处理文件数据
                result = self.process_file_data(buffer, current_file, 
current_file_obj, file_start_time, total_bytes)
                buffer, current_file, current_file_obj, file_start_time, 
total_bytes = result
                                
        except Exception as e:
            print(f"[Receiver] Error handling connection from {self.addr}: 
{e}")
        finally:
            if current_file_obj:
                if file_start_time:
                    ...
                    if duration > 0:
                        rate_mbps = (total_bytes * 8) / (duration * 
1000000)
                        print(f"[Receiver] File '{current_file}' closed: 
{total_bytes} bytes in {duration:.2f}s ({rate_mbps:.2f} Mbps)")
                    else:
                        print(f"[Receiver] File '{current_file}' closed: 
{total_bytes} bytes")
    ...
    def process_delay_probes(self, buffer):
        ...
        while True:
            ...
            # 提取完整的PING行
            ping_line = buffer[ping_start:newline_pos].decode('utf-8', 
errors='ignore')
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    delay_ms = (receive_time - timestamp_seconds) * 1000  
# 转换为毫秒
                    
                    # 【调试】前几个包打印详细时间戳信息
                    if probe_count < 5:
                        print(f"[Receiver] PING #{sequence}: 
send_time={timestamp_seconds:.6f}, "
                ...
                print(f"[Receiver] Error processing PING: {ping_line}, 
error: {e}")
        ...
        if probe_count > 0:
            ...
            if self.total_probe_count % 100 == 0:
                print(f"[Receiver] Processed {self.total_probe_count} 
delay probes from {self.addr}")
        
        return buffer
    
    def process_file_data(self, buffer, current_file, current_file_obj, 
file_start_time, total_bytes):
        """
        处理文件数据部分
        
        Returns:
            tuple: (buffer, current_file, current_file_obj, 
file_start_time, total_bytes)
        """
        while buffer:
            if not current_file:
                ...
                if b'FILE:' in buffer and b'\n' in buffer:
                    ...
                    newline_pos = buffer.find(b'\n', file_start)
                    header = 
buffer[file_start:newline_pos].decode('utf-8')
                    buffer = buffer[newline_pos+1:]
                    
                    if header.startswith('FILE:'):
                        filename = header[5:]
                        print(f"[Receiver] Starting to receive file 
'{filename}'")
...
当前正在接收文件，检查是否有新的文件头（表示当前文件结束）
                ...
                if file_start != -1:
                    ...
                    if duration > 0:
                        rate_mbps = (total_bytes * 8) / (duration * 
1000000)
                        print(f"[Receiver] Completed '{current_file}': 
{total_bytes} bytes in {duration:.2f}s ({rate_mbps:.2f} Mbps)")
                    else:
                        print(f"[Receiver] Completed '{current_file}': 
{total_bytes} bytes")
        ...
        return buffer, current_file, current_file_obj, file_start_time, 
total_bytes
...
def main():
    ...
    try:
        ...
        print("[Receiver] Warning: Could not enable MPTCP, using regular 
TCP")
